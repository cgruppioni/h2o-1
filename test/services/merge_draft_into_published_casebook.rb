...
class MergeDraftIntoPublishedCasebookTest < ActiveSupport::TestCase
  ...
  test 'remove deleted resources' do
  ...
  test 'updates published ordinals' do
  ...
  test 'adds newly created draft resources to published' do
  ...
  test 'merges unpublished revisions into published casebook' do
  ...
  test 'adds/merges new and updated annotations' do
  ...
  test 'removes deleted annotations from published' do
  ...
  test 'adds, deletes or modifies content collaborators' do
  ...
  test 'draft is destroyed after merge finished and published casebook is returned' do
